...
/*
// MAP
const sucos = frutas.map(function(fruta) {
    return "Suco de " + fruta;
});

sucos.forEach(suco => console.log(suco))

// FILTER
const frutaComecaComB = frutas.filter(fruta => fruta.startsWith('B'))
console.log(frutaComecaComB[0])

// FIND
const frutaMaisQuatroLetras = frutas.find(fruta => fruta.length > 4)
console.log(frutaMaisQuatroLetras)

// REDUCE
const salada = ""
const saladaDeFruta = frutas.reduce(
    (inicial, atual) => inicial + atual + " ",
    salada
)

console.log(saladaDeFruta)
*/